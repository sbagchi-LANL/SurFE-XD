...
def par_findiff_nonuniform(xn, yn):
    """findiff operation for piecewise-uniform data
    Args:
        xn: x coordinates of piecewise surface
        yn: y coordinates of piecewise surface
    ...
    d_dx = FinDiff(0, xn, acc=2)
    dy_dx = d_dx(yn)
    d2_x = FinDiff(0, xn, 2, acc=2)
    d2y_dx = d2_x(yn)
    k = d2y_dx/(1+dy_dx**2)**1.5
    s = 1./np.sqrt(1+dy_dx**2)
    return k, s

def curvature_normal_refined(x, y, smooth=False):
    ...
    if smooth is True:
        ...
        indx2 = [0]
        for i in indx[1:-1]:
            indx2 = np.concatenate((indx2, int(i-10), int(i+10)),axis=None)
        indx2 = np.append(indx2,indx[-1])
        indx = indx2
        #print(indx)
        for i in range(np.size(indx)-1):
            ...
            if i%2==1:
                k[indx[i]:indx[i+1]], s[indx[i]:indx[i+1]] = par_findiff_nonuniform(x_eff[indx[i]:indx[i+1]], y_eff[indx[i]:indx[i+1]])
    else:
        #print('curve not smooth')
        for i in range(np.size(indx)-1):
            k[indx[i]:indx[i+1]], s[indx[i]:indx[i+1]] = par_findiff(x_eff[indx[i]:indx[i+1]], y_eff[indx[i]:indx[i+1]])
...
def curvature_normal_nonuniform(x, y):
    ...
    k, s = par_findiff_nonuniform(x_eff, y_eff)
...
#     print ("N:", N)
    ...
    if i==1 :
        xlo = x_bot[N-int((N-1)*0.3)-1]
        xhi = x_bot[int((N-1)*0.3)+N]
        yhi = y_mid[int(np.size(y_mid)/8)]
    elif i==2:
        xlo = x_bot[N-int(N*0.3)]
        xhi = x_bot[N+int(N*0.3)]
        yhi = y_mid[int(np.size(y_mid)/6)]
    elif i==3:
        xlo = x_bot[N-int(N*0.2)]
        xhi = x_bot[N+int(N*0.2)]
        yhi = y_mid[int(np.size(y_mid)/10)]
    else:
...
def create_new_mesh_bot(k_mesh, lam, A, bottomsurf, xmax, xmin, ymax, ymin, surftype='gaussian'):
    ...
    if surftype == 'gaussian' and bottomsurf is None:
        b = lam/np.pi
        bottomsurf = 2*np.exp(-((x_vec-x1/2)/b)**2)*A + ymin
    elif surftype == 'sinusoidal' and bottomsurf is None:
        w = np.pi/lam
        bottomsurf = A * np.cos(2 * w * x_vec) + ymin
...
#    plt.savefig('E_field_'+repr(lam)+'.png', bbox_inches = 'tight')
    ...
    if symm is True:
        ...
        N_sb = (np.size(E_b))
        if N_sb % 2 == 0:
            E_b[int(N_sb/2):] = np.flip(E_b[:int(N_sb/2)])
        else:
            E_b[(int(N_sb/2)+1):] = np.flip(E_b[:int(N_sb/2)])
...
def fem_surface_evol(ds, e2, s2, d_om, gamma, perm, lam, dt):
...
def fdm_surface_evol(ds, e2, se, d_om, gamma, perm, lam, dt, smooth=False):
    ...
    k, s, indx = curvature_normal_refined(x_dof, y_dof, smooth=smooth)
    #print(indx)
    
    F = d_om*(-gamma*k - 0.5*perm*e2 + se)
    #F = d_om*(-gamma*k + s2)
    ...
    if smooth is True:
        ...
        for i in range(np.size(indx)-1):
            ...
            if i%2==1:
                d_dx = FinDiff(0, x_dof[indx[i]:indx[i+1]],acc=2)
                df_dx = d_dx(F[indx[i]:indx[i+1]])
                df_ds = df_dx*s[indx[i]:indx[i+1]]
                vfn[indx[i]:indx[i+1]] = d_dx(df_ds)
    else:
...
def fdm_surface_evol_nonuniform(ds, disp, se, d_om, gamma, perm, lam, dt):
    """Surface evolver routine using finite difference
    Args:
        ds: coordinates of surface profile (2D numpy array)
        e2: E^2 corresponding to the surface
    ...
    x_dof = np.concatenate((x_dof[-10:-1]-(x_dof[-1]-x_dof[0]), x_dof, x_dof[1:10]+(x_dof[-1]-x_dof[0])),axis=None)
    y_dof = np.concatenate((y_dof[-10:-1], y_dof, y_dof[1:10]),axis=None)
    e2 = np.concatenate((e2[-10:-1], e2, e2[1:10]),axis=None)
    se = np.concatenate((se[-10:-1], se, se[1:10]),axis=None)
    hd = np.concatenate((hd[-10:-1], hd, hd[1:10]),axis=None)
    vd = np.concatenate((vd[-10:-1], vd, vd[1:10]),axis=None)
    
    y_dof_relax = y_dof + vd
    x_dof_relax = x_dof + hd

    k, s = curvature_normal_nonuniform(x_dof_relax, y_dof_relax)
    #print(indx)
...
def fdm_surface_evol_sep(ds, disp, e2, se, d_om, gamma, perm, lam, dt):
    """Surface evolver routine using finite difference
    Args:
        ds: coordinates of surface profile (2D numpy array)
        e2: E^2 corresponding to the surface
    ...
    vft = np.zeros(np.size(x_dof))
    vfe = np.zeros(np.size(x_dof))
    vfs = np.zeros(np.size(x_dof))
    ...
    vfs = vfs[9:-9]
    k = k[9:-9]
    return xn, yn, vft, vfe, vfs, vfn, k
